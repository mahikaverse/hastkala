...
class VoiceStep2QuantityScreen extends StatefulWidget {
  ...
  const VoiceStep2QuantityScreen({super.key, required this.draft});

  @override
  State<VoiceStep2QuantityScreen> createState() => _VoiceStep2QuantityScreenState();
}

class _VoiceStep2QuantityScreenState extends State<VoiceStep2QuantityScreen>
  ...
  void _populateControllers(Map<String, dynamic> data) {
    if (data['quantity'] != null && data['quantity'].toString().trim().isNotEmpty) {
      _quantityCtrl.text = data['quantity'].toString().trim();
    }
    if (data['production_capacity'] != null && data['production_capacity'].toString().trim().isNotEmpty) {
      _capacityCtrl.text = data['production_capacity'].toString().trim();
    }
    if (data['making_time'] != null && data['making_time'].toString().trim().isNotEmpty) {
      _makingTimeCtrl.text = data['making_time'].toString().trim();
    }
    if (data['making_process'] != null && data['making_process'].toString().trim().isNotEmpty) {
      _makingProcessCtrl.text = data['making_process'].toString().trim();
    }
  }

  void _saveAndProceedToStep3() {
    final d = widget.draft;
    d.quantity = int.tryParse(_quantityCtrl.text.trim()) ?? d.quantity;
    d.productionCapacity = _capacityCtrl.text.trim().isNotEmpty ? _capacityCtrl.text.trim() : d.productionCapacity;
    d.makingTime = _makingTimeCtrl.text.trim().isNotEmpty ? _makingTimeCtrl.text.trim() : d.makingTime;
    d.makingProcess = _makingProcessCtrl.text.trim().isNotEmpty ? _makingProcessCtrl.text.trim() : d.makingProcess;

    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => VoiceStep3StoryScreen(draft: widget.draft),
      ),
    );
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // EXTRACTED FORM VIEW (Editable Form for Step 2)
  // ═══════════════════════════════════════════════════════════════════════════
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(
          ...
          height: 52,
          child: ElevatedButton(
            onPressed: _saveAndProceedToStep3,
            style: ElevatedButton.styleFrom(
              backgroundColor: AppColors.terracotta,
              foregroundColor: Colors.white,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppDimensions.radiusFull)),
  ...
  Widget _buildFormField(String label, TextEditingController controller, IconData icon, String hint, {TextInputType keyboardType = TextInputType.text, int maxLines = 1}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
